...
public class EstoqueProdutos {
	...
	public void calcularValorTotalEstoque() {
		int quantidadeTotal = produtos.entrySet().stream().mapToInt(p -> p.getValue().getQuantidade()).sum();
		double valorTotal = produtos.entrySet().stream().mapToDouble(p -> p.getValue().getPreco()).sum();
